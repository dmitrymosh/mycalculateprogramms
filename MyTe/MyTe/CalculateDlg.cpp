...
void CCalculateDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	{		
		...
		for(int i=0;i<nItems;i++)
		{
			...
			theApp.FileList.Files[i]=lpszFile;
		}
	...
	else
	{
		...
		if(count!=0)
		{
			...
			{
				int pos = (int)hListBox->SendMessage(LB_ADDSTRING, 0, (LPARAM) theApp.FileList.Files[i].GetString()); 
	...
	DDX_Text(pDX, IDC_OUTPUT_FILE,theApp.FileList.OutFile);
	DDX_Text(pDX, IDC_VEGA_FILE,theApp.FileList.VegaFile);
}
